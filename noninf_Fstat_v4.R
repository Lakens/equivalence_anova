...
#  eta_pop<-(Fstat*df1)/(Fstat*df1+df2)  # not sure about this?
#  ncp = N/(1/eta_pop-1)
...
dat<-rmvnorm(n1, mean=c(mu1,mu2,mu3), sigma=matrix(c(sigma2e,COR,COR,COR, sigma2e, COR,COR, COR, sigma2e),3,3))
...
ttt<-(summary(aov(y~a +Error(subject), data= dat)))$"Error: Within" 
...
simresults <- apply(cbind(1:nSim),1, 
	...
	# Run a non-inferiority test with delta=0.399
...
## Is our point estimate for Lambda (i.e. ncp_hat) unbiased?  (true ncp=200)
mean(simresults["ncp_hat",], na.rm=TRUE)
median(simresults["ncp_hat",], na.rm=TRUE)
hist(simresults["ncp_hat",])
...
# Looks like the distribution of p-values under the null is not uniform.
