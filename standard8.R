...
pval<-uniroot(delta_dist, lower=0,upper=0.4999)$root
return(pval)
}}
...
# A 90% C.I. two-sided confidence interval for this standardized mean does
# not fall entirely within the equiv. margin::
...
# F.value=0.25; df.1=4; df.2=50; N=55; delta_upper=0.5;
# F.value= 2.043706; df.1=2; df.2=57; N=60; delta_upper=0.66;
	my_F <- F.value
	my_df.1 <- df.1
	my_df.2 <- df.2
	my_N <- N
...
absmin<-function(x){x[which.min(abs(x))]}
if(CI(0.0001)<delta_upper){print("pval<=0.0001"); return(0.001)}

if(CI(0.001)>=delta_upper){
...
# 95% C.I. one-sided confidence interval is entirely below 0.5:
ci.snr(F.value=3.25, df.1=4, df.2=50, N=55, alpha.lower=0.00, alpha.upper=0.05)

# A (1-noninf.pval)% C.I. one-sided confidence interval is exactly under 0.5:
...
## We simulate datasets with true pop sig-to-noise ratio of 0.6667
## WE test if pop sig-to-noise ratio is less than 0.65 (i.e. null just barely true)
...
if(prop_of_var_hat> delta_upper){print("prop_of_var_hat outside of margin");return(1)}
...
if(CI(0.001)<delta_upper){print("pval<=0.001"); return(0.001)}
...
qq<-0.99; while(is.na(CI(qq))){qq<-qq-0.001}  # This is needed because of potential for ci.snr() to output NAs with large values of "alpha.upper"


pval<-uniroot(delta_dist, lower=0.0000001,upper=qq)$root
...
## example with a fairly large dataset:
##########
...
# 95% one-sided confidence interval (!!ERROR if "alpha.lower=0.00" !!): 
...
# Non-inferiority test p-value (H0: phi_p^2>0.5 vs. H1: phi_p^2<=0.5)
noninf.pval <- pop_prop_of_var(F.value= ttt[[1]]$F[1], df.1= ttt[[1]]$Df[1], df.2= ttt[[1]]$Df[2], N=N, delta_upper=0.5)
noninf.pval
...
sqrt_pop_sig_to_noise<-function(F.value, df.1, df.2, N, delta_upper=+0.2){
# F.value=3.25; df.1=4; df.2=50; N=55; delta_upper=0.5;		
	my_F.value <- F.value
	my_df.1 <- df.1
	my_df.2 <- df.2
...
sqrt_pop_sig_to_noise_hat<-mean(unlist(ci.srsnr(F.value=my_F.value, df.1=my_df.1, df.2=my_df.2, N=my_N, alpha.upper=0.49, alpha.lower=0.49)[c(1,3)]))


if(sqrt_pop_sig_to_noise_hat> delta_upper){print("sqrt_pop_sig_to_noise_hat outside of margin");return(1)}
if(sqrt_pop_sig_to_noise_hat <= delta_upper){

CI<-function(x){unlist((ci.srsnr(F.value= my_F.value, df.1=my_df.1, df.2=my_df.2, N=my_N, alpha.upper=x, alpha.lower=0)))[2]}
...
qq<-0.999; while(is.na(CI(qq))){qq<-qq-0.001}  # This is needed because of potential for ci.snr() to output NAs with large values of "alpha.upper"


pval<-uniroot(delta_dist, lower=0.001,upper=qq)$root
...
sqrt_pop_sig_to_noise(5.5, 15, 14, 200, 0.85)
...
std_targeted_eff<-function(means, s.anova, c.weights, n, N,  delta_lower=0.5, delta_upper=+1.5, tol=0.001){
# means=c(2, 4, 9, 13); s.anova=.80; c.weights=c(.5, -.5, -.5, .5); 
# n=c(30, 30, 30, 30); N=120; delta_lower=0.5; delta_upper= 1.5; tol=0.001;
	my_means <- means
	my_s.anova <- s.anova
...
#print(mean(CI(0.49)))
if(mean(CI(0.49))> delta_upper | mean(CI(0.49))< delta_lower){
	print("phi_hat outside of margin");return(1)}else{
...
## example:
std_targeted_eff(means=c(2, 4, 9, 13), s.anova=.80, c.weights=c(.5, -.5, -.5, .5), n=c(30, 30, 30, 30), N=120, delta_lower=0.15, delta_upper= 1.25)
...
ci.sc(means=c(2, 4, 9, 13), s.anova=.80, c.weights=c(.5, -.5, -.5, .5), n=c(3, 3, 3, 3), N=12, conf.level=.95)


# simulate date with true pop.standardized targeted effect=1.5:
aovdata<-simdata(10)
dat<-aovdata$dat
groups<-aovdata$groups
N<-dim(aovdata)[1]
ttt<-(summary(aov(dat~groups)))
means1<-unique(round(fitted(lm(dat~groups)),5))
s.anova1<- sqrt((ttt)[[1]]$"Mean Sq"[2])
n1<-c(table(groups))
 
# 90% two-sided confidence interval: 
ci.sc(means=means1, s.anova=s.anova1, c.weights=c(-.5, -.5, 1), n=n1, N=length(dat),conf.level=0.95)

# within [-2,2] margin?

std_targeted_eff(means=means1, s.anova=s.anova1, c.weights=c(-.5, -.5, 1), n=n1, N=length(dat),delta_lower=-2, delta_upper= 2)





# simulate date with true pop.standardized targeted effect=0.5:
aovdata<-simdata(10, sigma2e=9)
dat<-aovdata$dat
groups<-aovdata$groups
N<-dim(aovdata)[1]
ttt<-(summary(aov(dat~groups)))
...
ci.sc(means=means1, s.anova=s.anova1, c.weights=c(-.5, -.5, 1), n=n1, N=length(dat),conf.level=0.95)
# within [-1,1] margin?
std_targeted_eff(means=means1, s.anova=s.anova1, c.weights=c(-.5, -.5, 1), n=n1, N=length(dat),delta_lower=-1, delta_upper= 1)
...
simresults <- apply(cbind(1:nSim),1, 
	function(x){
	   ...
	   s.anova1<- sqrt((ttt)[[1]]$"Mean Sq"[2])
		n1<-c(table(groups))
		pval<-std_targeted_eff(means=means1, s.anova=s.anova1, c.weights=c(-.5, -.5, 1), n=n1, N=N1,delta_lower=-1.49, delta_upper= 1.49)
		return(pval<0.05)
				} 
			)
